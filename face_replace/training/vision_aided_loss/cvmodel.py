...
from face_replace.training.vision_aided_loss.DiffAugment_pytorch import DiffAugment
...
class DINOv2(torch.nn.Module):
    ...
    def __call__(self, x):
        ...
        if 'conv_multi_level' in self.cv_type:
            x = self.model.get_intermediate_layers(x, n=8, return_class_token=True)
            class_tokens, x = zip(*[(out[1], out[0]) for out in x])
            x = [x[i] for i in [0, 4, -1]]
            x[0] = x[0].permute(0, 2, 1).reshape(-1, 1024, 16, 16)
            x[1] = x[1].permute(0, 2, 1).reshape(-1, 1024, 16, 16)
            x[2] = class_tokens[-1]
        else:
            x = self.model(x)
...
class CVBackbone(torch.nn.Module):
    ...
    def __init__(self, cv_type, output_type, diffaug=False, device='cpu'):
        ...
        self.class_name_dict = {
                ...
                'dinov2': 'face_replace.training.vision_aided_loss.cvmodel.DINOv2',
